...
(The above method solves a problem with a >0, which is not allowed.)

Instead, we should define variables $y = \frac{1}{f^T x + g}$ and $z=y x$. (picture taken of solution)
This rewrite makes the objective writable as a linear program, then we can directly change relax inequality $y>0$ to $y\geq 0$. 
The new formulation is a relaxation
...
(CORRECTION:)
$r$ needs to be normalized in each direction. 
We need to multiply $a^T\frac{a_i}{\lVert a_i\rVert}$ to the $r$ term.
...
Need to take supremum in each direction, not just "largest $Q$".
...
(Can do this, alternatively, can use Caratheodory for conic combinations to be cleaner and more formal)
