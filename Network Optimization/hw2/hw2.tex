...
(Imagine increasing $y_V$ continuously. 
First constraint to get tight is cheapest edge.
Then, as we increase $y_V$ further, we somehow decrease $y_S$'s in a way that we cover all the already tight edges but also decrease the objective the least.)

(Variables with $|S|=1 $ do not contribute to the solution so can be set to some non-zero value. Need this argument somewhere)

(Give a dual solution with a laminar set of non-zero dual variables of maximum size.
Then we have like $n-1$ laminar tight constraints, so comparing tight constraints, we see they differ by adding a ?)
