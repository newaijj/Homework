...
Let $\calA$ be the event that $\calH$ is not 2-colorable. 
For a 2-coloring $C$ of $\calH$, let $\calB_C$ be the event that $C$ is proper.
For a given hyperedge $f$, let $\calC_{C,f}$ be the event that the edge $f$ is not monochromatic in $C$.
...
\end{lemma}
\begin{proof}
    By the independence of selecting each edge, MAYBE CANT DENOTE $f\in \calH$ LIKE THIS. INDEX EDGES INSTEAD
    \begin{align*}
        \Pr(\calB_C) = \Pr\left(\bigcap_{f\in \calH}\calC_{C,f}\right) = \prod_{f\in \calH}\Pr\left(\calC_{C,f}\right).
    \end{align*}
    Each coloring $C$ assigns at least $\frac{k^2}{2}$ vertices to one color $A$, so $|A|\geq \frac{k^2}{2}$. 
    For each hyperedge $f$,
    \begin{align*}
        \Pr(\calC_{C,f}) &= 1 - \Pr(f\text{ is monochromatic in }C) \\
        & \leq 1 - \Pr(\text{all vertices of }f\text{ are in }A) \\
    ...
    \end{align*}
    INEQUALITY THE WRONG WAY ON THE LAST STEP?
...
\begin{enumerate}
    \item $\Pr(\text{some coloring survives})\leq \sum_{\text{\#colorings}} \Pr (\text{a given coloring survives})$ (union bound)
    \item $\Pr(\text{a given coloring survives}) = \Pr(\text{the given coloring survives a given edge})^m$ (independence)
    \item $\Pr(\text{the given coloring survives a given edge})\leq (1-\frac{1}{2^k})$ (because there is always at least one color with at least half of the edges, and the hyperedge must at least avoid picking all its vertices from that half)
\end{enumerate}
